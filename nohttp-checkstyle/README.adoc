...
The nohttp project provides a https://github.com/spring-io/nohttp/tree/master/nohttp#whitelisted-http-urls[default whitelist].
...
If you prefer, you can embed a whitelist rather than externalizing it into a separate file by using the `whitelist` property.
The value of `whitelist` should be in the format of https://github.com/spring-io/nohttp/tree/master/nohttp#whitelisted-http-urls[RegexPredicate.createWhitelist(InputStream)].
For example, the following will whitelist `http://example.com` and `http://example.org`:
